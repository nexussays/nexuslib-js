...
PAGE OBJECTS
**********************/
function CreateMenu(id, buildFrom)
{
	if(buildFrom != null)
	{
		function recurse(menu)
		{
			...
			{
				var item = menu[x], type = objtype(item[1]), li = DOM.li(), match;
				
				if(type == "array")
				{
					li.append(DOM.span({ "class": "dropdown" }, item[0]), recurse(item[1]));
				}
				else if(item[1])
				{
					...
					{
						li.appendChild(DOM.a({"href":item[1]}, item[0]));
					}
				}
				//no children, just something visible in the menu section
				else
				{
					li.appendChild(DOM.span({"class":"text"}, item[0]));
				}
				ul.appendChild(li);
			}
			return ul;
